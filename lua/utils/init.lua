...
function M.get_root()
  ...
  path = path ~= '' and vim.ev.fs_realpath(path) or nil
  ---@type string[]
  local roots = {}
  if path then
  ...
  local root = roots[1]
  if not root then
    path = path and vim.fs.dirname(path) or vim.ev.cwd()
    ---@type string?
    root = vim.fs.find(M.root_patterns, { path = path, upward = true })[1]
    root = root and vim.fs.dirname(root) or vim.ev.cwd()
  end
  ---@cast root string
  return root
end
...
function M.telescope(builtin, opts)
  ...
  return function()
    ...
    if builtin == 'files' then
      if vim.ev.fs_stat((opts.cwd or vim.ev.cwd()) .. '/.git') then
    ...
    if opts.cwd and opts.cwd ~= vim.ev.cwd() then
